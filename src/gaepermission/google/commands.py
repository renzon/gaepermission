...
class FindMainUserFromGoogleUser(FindMainUserFromExternalUserId):
    def __init__(self, id):
        super(FindMainUserFromGoogleUser, self).__init__(GoogleUser.query_by_external_id(id))


class CheckMainUserGoogleEmailConflict(CheckMainUserEmailConflict):
    def __init__(self, email, external_id):
        super(CheckMainUserGoogleEmailConflict, self).__init__(email, FindMainUserFromGoogleUser(external_id))
    ...
    def do_business(self, stop_on_error=True):
        ...
        if self.result:
            if self.external_user is None and self.main_user_from_external is None:
                self.external_user = GoogleUser(external_id=self.google_api_user.user_id())
